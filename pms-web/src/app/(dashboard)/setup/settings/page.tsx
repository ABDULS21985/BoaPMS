...
import { getSettings, createSetting, updateSetting } from "@/lib/api/performance";
import type { Setting } from "@/types/performance";

const settingTypes = ["String", "Bool", "Int", "Long", "Float", "Double", "Decimal", "DateTime"];
...
export default function SettingsPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try { const res = await getSettings(); if (res?.data) setItems(Array.isArray(res.data) ? res.data : []); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = editItem
        ? await updateSetting({ settingId: editItem.settingId, ...formData })
        : await createSetting(formData);
      if (res?.isSuccess) { toast.success(editItem ? "Setting updated." : "Setting created."); setOpen(false); loadData(); }
  ...
  if (loading) return <div><PageHeader title="System Settings" breadcrumbs={[{ label: "Setup" }, { label: "Settings" }]} /><PageSkeleton /></div>;

  return (
    <div className="space-y-6">
      <PageHeader title="System Settings" description="Manage system settings with encrypted value support" breadcrumbs={[{ label: "Setup", href: "/setup/strategies" }, { label: "Settings" }]} actions={<Button size="sm" onClick={openAdd}><Plus className="mr-2 h-4 w-4" />Add Setting</Button>} />
      <DataTable columns={columns} data={items} searchKey="name" searchPlaceholder="Search settings..." />

      <FormSheet open={open} onOpenChange={setOpen} title={editItem ? "Edit Setting" : "Add Setting"} isEdit={!!editItem} editWarning="Update the selected setting below.">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.type} onValueChange={(v) => setFormData({ ...formData, type: v })}>
              ...
              <SelectContent>{settingTypes.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
            </Select>
          </div>
