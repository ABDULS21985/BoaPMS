...
import { getStaffCommitteeWorkProducts } from "@/lib/api/pms-engine";
...
export default function CommitteeWpEvaluationPage() {
    ...
    Promise.all([getStaffDetails(staffId), getStaffActiveReviewPeriod()])
      .then(async ([staffRes, periodRes]) => {
        ...
        if (periodRes?.data) {
          ...
          const wpRes = await getStaffCommitteeWorkProducts(staffId, periodRes.data.periodId);
  ...
  if (loading) return <div><PageHeader title="Committee WP Evaluation" breadcrumbs={[{ label: "Evaluation" }, { label: "Committee" }]} /><PageSkeleton /></div>;

  return (
    <div className="space-y-6">
      <PageHeader title={`Committee WP Evaluation: ${staffName}`} description={period ? `Review Period: ${period.name}` : ""} breadcrumbs={[{ label: "Evaluation", href: "/evaluation/direct-reports" }, { label: "Committee WP", href: "/evaluation/direct-reports" }, { label: staffName }]} />
      {workProducts.length > 0 ? (
        <DataTable columns={columns} data={workProducts} searchKey="name" searchPlaceholder="Search work products..." />
      ) : (
        <EmptyState icon={Users2} title="No Committee Work Products" description="No committee work products found for evaluation." />
      )}
    </div>
  );
}
