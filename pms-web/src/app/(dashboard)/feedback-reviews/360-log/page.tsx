...
export default function FeedbackLogPage() {
  ...
  const columns: ColumnDef<CompetencyReviewer>[] = [
    ...
    { accessorKey: "finalRating", header: "Final Rating", cell: ({ row }) => row.original.finalRating?.toFixed(2) ?? "—" },
  ...
  if (loading) return <div><PageHeader title="360 Feedback Log" breadcrumbs={[{ label: "360 Feedback", href: "/feedback-reviews" }, { label: "Feedback Log" }]} /><PageSkeleton /></div>;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[{ label: "360 Feedback", href: "/feedback-reviews" }, { label: "Feedback Log" }]}
      />
      {logs.length > 0 ? (
