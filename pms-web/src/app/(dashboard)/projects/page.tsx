...
import { Plus, Eye, Loader2, FolderKanban } from "lucide-react";
...
import { getProjects, saveDraftProject } from "@/lib/api/pms-engine";
import { getReviewPeriods } from "@/lib/api/review-periods";
import { getDepartments } from "@/lib/api/organogram";
import type { Project, PerformanceReviewPeriod } from "@/types/performance";
...
export default function ProjectsPage() {
  ...
  const [formData, setFormData] = useState({ name: "", description: "", deliverables: "", reviewPeriodId: "", departmentId: "", projectManager: "", startDate: "", endDate: "" });

  const loadData = async () => {
    setLoading(true);
    try {
      const [projRes, periodRes, deptRes] = await Promise.all([getProjects(), getReviewPeriods(), getDepartments()]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await saveDraftProject({ ...formData, departmentId: formData.departmentId ? Number(formData.departmentId) : undefined });
      if (res?.isSuccess) { toast.success("Project created as draft."); setOpen(false); loadData(); }
      else toast.error(res?.message || "Save failed.");
    } catch { toast.error("An error occurred."); } finally { setSaving(false); }
  };

  const openAdd = () => { setFormData({ name: "", description: "", deliverables: "", reviewPeriodId: "", departmentId: "", projectManager: "", startDate: "", endDate: "" }); setOpen(true); };
  ...
  if (loading) return <div><PageHeader title="Projects" breadcrumbs={[{ label: "Projects" }]} /><PageSkeleton /></div>;

  return (
    <div className="space-y-6">
      <PageHeader title="Projects" description="Manage all projects across the organization" breadcrumbs={[{ label: "Projects" }]} actions={<Button size="sm" onClick={openAdd}><Plus className="mr-2 h-4 w-4" />New Project</Button>} />
      {projects.length > 0 ? (
        <DataTable columns={columns} data={projects} searchKey="name" searchPlaceholder="Search projects..." />
      ) : (
        <EmptyState icon={FolderKanban} title="No Projects" description="No projects have been created yet." />
      )}

      <FormSheet open={open} onOpenChange={setOpen} title="New Project" className="sm:max-w-lg overflow-y-auto">
        <div className="space-y-4">
          ...
          <div className="space-y-2"><Label>Project Manager</Label><Input value={formData.projectManager} onChange={(e) => setFormData({ ...formData, projectManager: e.target.value })} placeholder="Staff ID" /></div>
