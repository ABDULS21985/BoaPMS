...
export default function StaffRatingPage() {
  ...
  const handleSaveCompetencyRating = async () => {
    ...
    if (unAnswered.length > 0) { toast.error("Please answer all questions before saving."); return; }
    ...
    try {
      ...
      for (const q of questions) {
        ...
        const res = existingRating ? await update360Rating(payload) : await add360Rating(payload);
  ...
  const handleFinalize = async () => {
    try {
      ...
      if (res?.isSuccess) {
        ...
        setIsReviewComplete(true);
        router.push("/feedback/to-review");
      } else {
  ...
  if (loading) return <div><PageHeader title="Staff Rating" breadcrumbs={[{ label: "Feedback" }, { label: "To Review", href: "/feedback/to-review" }, { label: "Rating" }]} /><PageSkeleton /></div>;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "Feedback" },
          { label: "To Review", href: "/feedback/to-review" },
